...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, weight=False):
        ...
        if weight:
            attn_output_weights = attn.sum(dim=1) / self.n_head
            return attn_output_weights.permute(0,2,1)
...
class IBRNet(nn.Module):
    ...
    def forward(self, rgb_feat,  rgb_feat_delta, ray_diff, mask, flag): # 
        '''
        ...
        '''
        n_rays, n_samples, num_views, feat = rgb_feat.size()
        ...
        if not self.args.no_color_correction:
            rgb_in[:,:,num_views//2-window:num_views//2+1+window,...] = rgb_feat_delta[..., :3]
        rgb_feat = rgb_feat + direction_feat
       
        timestamp = torch.tensor(self.args.neighbor_list).cuda()
        alpha = 0.3
        weight_time1 = torch.exp(-alpha*timestamp.abs()).unsqueeze(0).unsqueeze(0).unsqueeze(-1).repeat(n_rays, n_samples, 1, 1)

        weight_time = weight_time1* mask 
        weight_time = weight_time / (torch.sum(weight_time, dim=2, keepdim=True) + 1e-8)
        ...
        x = self.base_fc(x) # (3*n_feat, 64) (64, 32)

        x_vis = self.vis_fc(x * weight) #(32, 32) (32, 33)
        ...
        # sigma_out = torch.concat([sigma_out, mask[:,0:1,num_views//2,:]], dim=-2)

        # rgb computation
        dot = weight.masked_fill(weight == 0, -1)
        ray_diff = torch.concat([ray_diff[...,:3],dot],dim=-1)
        # ray_diff = weight.repeat(1,1,1,4)
        ...
        blending_weights_key = torch.zeros_like(blending_weights_valid)
        blending_weights_key[:,:,num_views//2:num_views//2+1,:] += 1 
        blending_weights_valid = blending_weights_key.where(mask[...,num_views//2,:].unsqueeze(-2).repeat(1,1,num_views,1)==1, other=blending_weights_valid)
